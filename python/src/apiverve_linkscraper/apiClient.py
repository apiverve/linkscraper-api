...
class LinkscraperAPIClientError(Exception):
...
class ValidationError(LinkscraperAPIClientError):
...
class LinkscraperAPIClient:
    ...
    VALIDATION_RULES = {"url": {"type": "string", "required": True, "format": "url"}, "maxlinks": {"type": "number", "required": True, "default": "50"}, "includequery": {"type": "boolean", "required": False, "default": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the LinkscraperAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises LinkscraperAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise LinkscraperAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[LinkscraperAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns LinkScraperData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises LinkscraperAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise LinkscraperAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise LinkscraperAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise LinkscraperAPIClientError(f"Request failed: {str(e)}")
